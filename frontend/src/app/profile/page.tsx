        ...
        {account && isCorrectChain && <>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center pt-2 border-t border-[#1A2A1A]">
                <div>
                  ...
                  <span className="text-xl font-mono text-[#00FF41] font-bold">
                    {Number(referralBalance).toFixed(6)} ETH
                  </span>
                </div>
                {Number(referralBalance)>0 && (
                  <NeonButton onClick={withdraw} loading={loading} variant="gold">CLAIM</NeonButton>
                )}
              </div>
              <div className="pt-2 border-t border-[#1A2A1A]">
                ...
                <div className="flex gap-2">
                  <input
                    type="text" readOnly
                    value={`https://altcoin-fantasy.vercel.app/play?ref=${account}`}
                    className="flex-1 px-3 py-1.5 rounded bg-[#0A0A0F] border border-[#1A2A1A] text-[10px] text-[#4D754D] font-mono"
                  />
                  <button
                    onClick={()=>{navigator.clipboard.writeText(`https://altcoin-fantasy.vercel.app/play?ref=${account}`);setMessage('Copied!');}}
                    className="px-3 py-1.5 rounded border border-[#1A2A1A] text-[10px] font-mono text-[#4D754D] hover:text-[#00FF41] hover:border-[#00FF41]/50 transition-all"
                  >
                    COPY
                  </button>
                </div>
          ...
          <HudFrame variant="gold" title="ACHIEVEMENTS">
            <div className="grid grid-cols-2 gap-2">
              {[
                {name:'First Draft',unlocked:myDrafts.length>0,desc:'Submit first draft'},
                {name:'Captain',unlocked:myDrafts.some((d:any)=>d.captain_index!==0),desc:'Use captain boost'},
                {name:'Winner',unlocked:false,desc:'Top 10% finish'},
                {name:'Referrer',unlocked:Number(referralBalance)>0,desc:'Earn referrals'}
              ].map(a=>(
                <div key={a.name} className={`p-3 rounded border text-center text-[10px] font-mono transition-all ${
                  a.unlocked
                    ? 'border-[#FFD700]/40 bg-[#1A1A0A]/50 text-[#FFD700] neon-box-gold'
                    : 'border-[#1A2A1A] bg-[#0A0A0F] text-[#4D754D]'
                }`}>
                  <div className="font-bold text-xs mb-0.5">{a.unlocked?'◆':'◇'} {a.name}</div>
                  <div className="opacity-60">{a.desc}</div>
                </div>
              ))}
            </div>
          </HudFrame>
